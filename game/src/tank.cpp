...
public:
    ...
    void render()
    {
        ...
        // engine->render_line(attr, matrices.single_matrix(),
        // matrices.single_matrix(),matrices.single_matrix(), ind);
    ...
    void update(vector<obstacle>& objects)
    {
        ...
        if (!isDead)
        {
            ...
            {
                float exposion_x = (cos(turret_angle) * 0.25f + tank_center_x) *
                                   normolize_matrix[0];
                float exposion_y = (sin(turret_angle) * 0.25f + tank_center_y) *
                                   normolize_matrix[4];
    ...
    {
        float left_down_x = ((tank_center_x)*normolize_matrix[0]) -
                            0.2f * (1 / scaling_coefficient);
        float left_down_y = ((tank_center_y)*normolize_matrix[4]) -
                            0.1f * (1 / scaling_coefficient);
        ...
        float left_up_x = ((tank_center_x)*normolize_matrix[0]) -
                          0.2f * (1 / scaling_coefficient);
        float left_up_y = ((tank_center_y)*normolize_matrix[4]) +
                          0.1f * (1 / scaling_coefficient);
        ...
        float right_down_x = ((tank_center_x + d_x) * normolize_matrix[0]) +
                             0.1f * (1 / scaling_coefficient);
        float right_down_y = ((tank_center_y + d_y) * normolize_matrix[4]) -
                             0.1f * (1 / scaling_coefficient);
        ...
        float right_up_x = ((tank_center_x + d_x) * normolize_matrix[0]) +
                           0.1f * (1 / scaling_coefficient);
        float right_up_y = ((tank_center_y + d_y) * normolize_matrix[4]) +
                           0.1f * (1 / scaling_coefficient);
        ...
        float x = ((tank_center_x + d_x) * normolize_matrix[0]) +
                  0.1f * (1 / scaling_coefficient);
        ...
        float _x = ((tank_center_x + d_x) * normolize_matrix[0]) -
                   0.2f * (1 / scaling_coefficient);
                                    ...
                                    vector<obstacle> objects)
    {
             ...
             i++) // Обработка коллизий танка со стенами
        {
            if (objects[i].exist)
            {
                ...
                float obj_left_down_x =
                    (object_center_x - 0.1f * normolize_matrix[0]);
                float obj_left_down_y =
                    (object_center_y - 0.1f * normolize_matrix[4]);

                float obj_right_down_x =
                    (object_center_x + 0.1f * normolize_matrix[0]);

                float obj_right_up_y =
                    (object_center_y + 0.1f * normolize_matrix[4]);
    ...
    {
        float exposion_x =
            (cos(turret_angle) * 0.25f + tank_center_x) * normolize_matrix[0];
        float exposion_y =
            (sin(turret_angle) * 0.25f + tank_center_y) * normolize_matrix[4];
        explosion_animation.add_animation(exposion_x, exposion_y, 1);
        missiles.add_missile_to_stack(
            tank_center_x, tank_center_y, turret_angle);
    }
...
private:
    ...
    void rotate(float& x, float& y, float cx, float cy, float angle)
    {
        ...
        x = (_x * cos(angle) - _y * sin(angle)) * normolize_matrix[0] *
            scaling_coefficient;
        y = (_x * sin(angle) + _y * cos(angle)) * normolize_matrix[4] *
            scaling_coefficient;
        x += cx;
        y += cy;
    }
