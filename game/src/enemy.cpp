...
public:
    ...
    void update(vector<obstacle>& objects)
    {
        ...
        if (!isDead)
        {
            ...
            if (time - previos_intrepolation_base_time >
                    (unsigned int)(rand() % 5000 + 2500) &&
                !have_obstacle)
            {
                int random = rand() % 3;
        ...
        else
        {
            ...
            {
                float obstacle_color =
                    0.45f + ((time - previos_time) / 20000.f) * (0.55f);
...
private:
    ...
    // vector<float> angiles = {
    // 0.f,0.79f, 1.57f, 2.36f, 3.14f, 3.93f, 4.71f,5.5f };
    ...
    {
        float left_down_x = ((tank_x + d_x) * normolize_matrix[0]) -
                            0.2f * (1 / scale_coefficient);
        float left_down_y = ((tank_y + d_y) * normolize_matrix[4]) -
                            0.1f * (1 / scale_coefficient);
        ...
        float left_up_x = ((tank_x + d_x) * normolize_matrix[0]) -
                          0.2f * (1 / scale_coefficient);
        float left_up_y = ((tank_y + d_y) * normolize_matrix[4]) +
                          0.1f * (1 / scale_coefficient);
        ...
        float right_down_x = ((tank_x + d_x) * normolize_matrix[0]) +
                             0.1f * (1 / scale_coefficient);
        float right_down_y = ((tank_y + d_y) * normolize_matrix[4]) -
                             0.1f * (1 / scale_coefficient);
        ...
        float right_up_x = ((tank_x + d_x) * normolize_matrix[0]) +
                           0.1f * (1 / scale_coefficient);
        float right_up_y = ((tank_y + d_y) * normolize_matrix[4]) +
                           0.1f * (1 / scale_coefficient);
        ...
        float x = ((tank_x + d_x) * normolize_matrix[0]) +
                  0.1f * (1 / scale_coefficient);
        ...
        float _x = ((tank_x + d_x) * normolize_matrix[0]) -
                   0.2f * (1 / scale_coefficient);
...
ITank* create_enemy_tank(IEngine*      eng,
                         ...
                         int*          score_)
{
    return new enemy(eng, tank, x, y, angle, norm, coeff, score_);
}
